...
from core.configuration_property import ListItem
...
def get_win_version(
    ...
    """
    succeed, output = process_runner.run_in_wine_prefix(
        ExecutableCommand("winecfg", "/v"), runtime_configuration, logger, True
    )
        ...
        for line in output.splitlines():
            ...
            for version, description in WIN_VERSIONS.items():
                if line.startswith(version):
                    logger.info("Current Windows version in Wine is %s", description)
                    return version
...
def set_win_version(
    version: str, runtime_configuration: RuntimeConfiguration, logger: logging.Logger
) -> None:
    """
